 ...
 * @return Success or failure
 */
bool AD5934::setSettlingCycles(int time) { //for clock and synchronisation
    int cycles;
 ...
 * @return Success or failure
 */
bool AD5934::setIncrementFrequency(unsigned long increment) { //for setting how much frequencies are changed and incremented during frequency sweep
 ...
 * @return Success or failure
 */
bool AD5934::setNumberIncrements(unsigned int num) { //til when the sweep goes, how much increment steps it takes
    if (num > 511) {
        return false;
    }
 ...
 * @return Success or failure
 */
bool AD5934::setPGAGain(byte gain) { //choses feedback resistor based on setting this bit- either 1 or 0 => 1 or 5, for the amplifier inside the chip
    byte val;
    if (!getByte(CTRL_REG1, &val))
        return false;
...
bool AD5934::setRange(byte range) { //for setting the output voltage
    byte val;
...
bool AD5934::calibrate(double gain[], int phase[], int ref, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        phase[i] = atan2(imag[i], real[i]) * (180.0 / M_PI);  // Convert to degrees if needed /find value of i
...
/*-------------------------------------------------------------Data Send------------------------------------------------------------------*/
void sendDataToRaspberryPi(float phaseData[], float temperature, float humidity) {
    for(int i=0;i<NUM_INCREMENTS;i++){
        Serial.print(phaseData[i], 3);
